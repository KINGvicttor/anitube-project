        ...
        <section className="w-full h-[calc(100vh-100px)] bg-radial from-primary to-secondary from-20% ">
            <div className="container mx-auto w-full h-full">
                ...
                <div className="flex flex-wrap w-full gap-1 ">
                    {dataCtx?.mangasData.slice(0, 12).map(manga => (
                        <div key={manga.id} className=" w-[250px] h-[250px]">
                            <div className="w-full h-full rounded-lg overflow-hidden">
                                <img className="w-full h-full rounded-lg bg-cover bg-center cursor-pointer hover:scale-110 hover:duration-1000" src={manga.attributes.posterImage.original} />
                            </div>
                            <div className="w-full h-full relative mt-[-250px] rounded-lg bg-gradient-to-t pointer-events-none from-black to-transparent flex items-end">
                                <p className="m-4">{manga.attributes.titles.en_jp}</p>
                            </div>
                        </div>
                    ))}
                </div>
